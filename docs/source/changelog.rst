...
* The ``module`` keyword argument to ``EnvSource.env_name()``. ``.get()``
  deprecated it in v0.4.0 and ``env_name()`` took it without a warning.
  Use ``path``; both are removed in v0.5.0.
